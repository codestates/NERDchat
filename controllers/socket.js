...
module.exports = {
  nspSocket: async (socket) => {
    ...
    socket.on("joinVoice", (roomUid) => {
      ...
      const usersInRoom = roomUsers[roomUid].filter(
        (id) => id !== socket.userId
      );
      ns.to(roomUid).emit("allUsers", usersInRoom);
    });
    socket.on("sending signal", (payload) => {
      ns.to(payload.userToSignal).emit("userJoin", {
        signal: payload.signal,
        callerId: payload.callerId,
      });
    });
    socket.on("return signal", (payload) => {
      ns.to(payload.callerId).emit("receive return signal", {
        signal: payload.signal,
        id: socket.userId,
      });
    });
    ...
    const users = [];
    const [tokens] = await Promise.all([tokenStore.findAllToken()]);
